...
# This function extracts department information from the "Authors with affiliations" column.
def process_department_stats(file):
    ...
    for index, row in df.iterrows():
        ...
        # Split the affiliation string by semicolon in case there are multiple affiliations
...
with tab2:
    ...
    st.write("This module calculates the number of research papers and total citations for each department by matching department names within the 'Authors with affiliations' column. Citation counts are taken from the 'Cited by' column.")
    if uploaded_file:
